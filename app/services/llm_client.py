...
class LLMCompanyAnalyzer(BaseLLMGenerator):
    """Клиент для анализа интервью отдельных компаний"""
    def __init__(self):
        super().__init__(
            config=settings.llm_company_analyzer,
            system_prompt=SystemPrompts.company_interview_analyzer
        )


class LLMHypothesesGenerator(BaseLLMGenerator):
    """Клиент для анализа интервью отдельных компаний"""
    def __init__(self):
        super().__init__(
            config=settings.llm_hypotheses_generator,
            system_prompt=SystemPrompts.hypotheses_generator
        )


class LLMIndustryAnalyzer(BaseLLMGenerator):
    """Клиент для анализа интервью в рамках отрасли"""
    def __init__(self):
        super().__init__(
            config=settings.llm_industry_analyzer,
            system_prompt=SystemPrompts.industry_interview_analyzer
        )


class LLMFinalAnalyzer(BaseLLMGenerator):
    """Валидатор дерева диалогов"""
    def __init__(self):
        super().__init__(
            config=settings.llm_final_analyzer,
            system_prompt=SystemPrompts.final_interview_analyzer
        )
...
class LLMClients:
    def __init__(self):
        ...
        self.company_analyzer = LLMCompanyAnalyzer()
        self.hypotheses_generator = LLMHypothesesGenerator()
        self.industry_analyzer = LLMIndustryAnalyzer()
        self.final_analyzer = LLMFinalAnalyzer()
